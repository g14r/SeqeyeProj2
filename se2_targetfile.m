function se2_targetfile(SubCode, GroupCode,genchunks, CMB)
    ...
    for e= 1:length(Horizon)
        ...
        for i = 1:length(Trials)
            if ElimChunkStart
                ...
                while length(unique([seq(1:2) ; Chunks(:,1:2)] , 'rows')) <= length(Chunks) | (sum(tempSeq == 0) > 1 & sum(tempSeq2 == 0) > 0)| (sum(tempSeq == 1) > 1 & sum(tempSeq2 == 0) > 0) | (sum(tempSeq == -1) > 1 & sum(tempSeq2 == 0) > 0)
                ...
                tempSeq = diff(seq);
                tempSeq2 = diff(tempSeq);
                while (sum(tempSeq == 0) > 1 & sum(tempSeq2 == 0) > 0)| (sum(tempSeq == 1) > 1 & sum(tempSeq2 == 0) > 0) | (sum(tempSeq == -1) > 1 & sum(tempSeq2 == 0) > 0)
                    seq = sample_wor([1:5],1,MaxPress);
                    tempSeq = diff(seq);
                    tempSeq2 = diff(tempSeq);
                end
...
if ~ repeating    
    ...
    for e= 1:length(Horizon)
        ...
        Intermixed.Horizon(1:length(Trials),:) = Horizon(e);
...
%     StimTimeLim = zeros(length(Trials) , 1);
    ...
    for e= 1:length(Horizon)    
        ...
        for i = 1:length(X)
            ...
            ChunkArrangeLearn.Horizon(i,:) = Horizon(e);
            ChunkArrangeLearn.StimTimeLim(i,:) = StimTimeLim(1);
        ...
        for i = 1:length(rTrials)
            ...
            RandomSeq.hand(i,:) = 2;
            RandomSeq.sounds(i,:) = 1;
            RandomSeq.Horizon(i,:) = Horizon(e);
            RandomSeq.StimTimeLim(i,:) = StimTimeLim(1);
            for press= 1:MaxPress
                comnd  = [' RandomSeq.press' , num2str(press) , '(i,1) = seq(press);'];
                eval(comnd);
            end
