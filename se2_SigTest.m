...
%         ylabel('msec')
    ...
    case 'RT'
        ...
        if length(subjnum) == 1
            stats = anovan(ANA.RT,var,'model','interaction','varnames',FCTR , 'display' , 'off') ; % between subject;
        else
...
%             FCTR(strcmp(FCTR,'Horizon')) = {'Window'};
    ...
    case 'PercentLearning_CompareSeqType'
        
        dayz = unique(ANA.Day);
        D = ANA;
        ANA  = tapply(D , {'Horizon' , 'Day' ,'SN' , 'seqNumb'} , {'MT' , 'nanmedian(x)'});
    ...
    case 'PercentIPIplace'
        ...
        dayz = unique(ANA.Day);
        ipiss = ANA.IPI;
        SN = unique(ANA.seqNumb);

        A.prsnumb= reshape(repmat([1:13] , length(ipiss) , 1) , numel(ipiss) , 1);
        A.IPI = reshape(ipiss , numel(ipiss) , 1);
        A.Horizon = repmat(ANA.Horizon , size(ipiss , 2) , 1);
        A.Day = repmat(ANA.Day , size(ipiss , 2) , 1);
        A.seqNumb = repmat(ANA.seqNumb , size(ipiss , 2) , 1);
        ...
        A.IPIPlace = ones(size(A.prsnumb));
        ipiOfInterest = {[1:3] , [5:9] , [11:13]};
        for n = 1:length(ipiOfInterest)
            A.IPIPlace(ismember(A.prsnumb , ipiOfInterest{n})) = n;
        end
        ...
        for d = 2:length(dayz)
            for sn = 1:length(SN)
                for ip = 1:length(ipiOfInterest)
                    Db1= getrow(ANA , ANA.Day == 1 & ANA.IPIPlace==ip & ANA.seqNumb == SN(sn));
                    Db_d = getrow(ANA , ANA.Day == dayz(d) & ANA.IPIPlace==ip & ANA.seqNumb == SN(sn));
        ...
        stats = anovaMixed(Daybenefit.percChangeIPI  , Daybenefit.SN ,'within',var ,FCTR,'intercept',1) ;   
    case 'PercentLearning_Overall_withinseqType'
        % day 1 has to be included
        ANA = getrow(Dall , Dall.isgood & ~Dall.isError & ...
            ismember(Dall.Horizon , Horizon) & ...
            ismember(Dall.Day , [1 , Day]) & ismember(Dall.seqNumb , seqNumb) &ismember(Dall.SN , subjnum));
    ...
    case 'PercentLearning_Partial_withinseqType'
        % day 1 has to be included
        ANA = getrow(Dall , Dall.isgood & ~Dall.isError & ...
            ismember(Dall.Horizon , Horizon) & ...
            ismember(Dall.Day , [1 , Day]) & ismember(Dall.seqNumb , seqNumb) &ismember(Dall.SN , subjnum));
        ...
        for d = 1:length(dayz)-1
            for sn = 0:1
                Db1= getrow(ANA , ANA.Day == dayz(d) & ANA.seqNumb==sn);
                Db_d = getrow(ANA , ANA.Day == dayz(d+1) & ANA.seqNumb==sn);
                Db_d.percChangeMT = 100*abs((Db1.MT - Db_d.MT)./Db1.MT);
                Daybenefit = addstruct(Daybenefit , Db_d);
            end
        end
