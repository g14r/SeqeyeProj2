...
while(c<=length(varargin))
    switch(varargin{c})
        ...
        case {'whatIPI'}
            ...
            c=c+2;
        case {'ipiOfInterest'}
            % IPI of interest to test --> Steady State =[4:10]
            eval([varargin{c} '= varargin{c+1};']);
            c=c+2;
            ipiOfInterest
        otherwise
    ...
    case 'IPI'
        switch whatIPI
            ...
            case 'WithinBetweenChunk'
                ...
                mm= input('Pool [Random and Between (1)] , [within and between (2) , [nothing (0)]]?');
...
%                 anovan(A.IPI,var,'model','interaction','varnames',FCTR)  % between subject
                figure('color' , 'white')
                lineplot(fliplr(var), A.IPI , 'style_shade' , 'markertype' , 'o'  , ...
                    'markersize' , 10 , 'markerfill' , 'w');
                tAdd = FCTR{1};
                for f =2:length(FCTR)
                    tAdd = [tAdd , ' and ' , FCTR{f}];
