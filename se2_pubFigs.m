...
%         Hex = input('What horizons to exclude? (0 = include all)');
        ...
        for d = 1:length(dayz)
            ...
            [coo1(:,d),plot1(:,d),err1(:,d)] = lineplot([MT.Horizon] , MT.MT , 'subset' , ismember(MT.seqNumb , [1])  & ismember(MT.Day , dayz{d}));
            [coo0(:,d),plot0(:,d),err0(:,d)] = lineplot([MT.Horizon] , MT.MT , 'subset' , ismember(MT.seqNumb , [0]) & ismember(MT.Day , dayz{d}));
        ...
        switch nowWhat
            ...
            case 'Actual&fitDayz'
                ...
                for d = 1:length(dayz)
                    ...
                    [coo1_red(:,d),plot1_red(:,d),err1_red(:,d)] = lineplot([ANA.Horizon] , ANA.MT );
                    [coo1_pred_red(:,d),plot1_pred_red(:,d),err1_pred_red(:,d)] = lineplot([ANA.Horizon] , ANA.MT_pred );
                    
                    
                    ANA = getrow(MT , ismember(MT.seqNumb , [0])  & ismember(MT.Day , d));
                    ANA.Horizon(ANA.Horizon>5) = 5;
                    [coo0_red(:,d),plot0_red(:,d),err0_red(:,d)] = lineplot([ANA.Horizon] , ANA.MT );
                    [coo0_pred_red(:,d),plot0_pred_red(:,d),err0_pred_red(:,d)] = lineplot([ANA.Horizon] , ANA.MT_pred);
                end
                close(h1)
                scol = [200 200 200]/255;
                ecol = [0, 0 0]/255;
                ...
                for i = [1:length(unique(ANA.Horizon))]
                    plotshade((i-1)*(length(dayz)+1)+[1:length(dayz)],plot1_red(i,:),err1_red(i,:),'transp' , .6 , 'patchcolor' , horzcolor(:,i) , 'linecolor' , colz{6-i+1,2} , 'linewidth' , 3);
                    plot((i-1)*(length(dayz)+1)+[1:length(dayz)],plot1_red(i,:) , '-o' , 'MarkerSize' , 10 , 'color' , colz{6-i+1,2},'MarkerFaceColor',colz{6-i+1,2} , 'LineWidth' , 3);
                    
                    plotshade((i-1)*(length(dayz)+1)+[1:length(dayz)],plot0_red(i,:),err0_red(i,:),'transp' , .6 , 'patchcolor' , horzcolor(:,i) , 'linecolor' , colz{6-i+1,1} , 'linewidth' , 3);
                    plot((i-1)*(length(dayz)+1)+[1:length(dayz)],plot0_red(i,:) , '-o' , 'MarkerSize' , 10 , 'color' , colz{6-i+1,1},'MarkerFaceColor',colz{6-i+1,1} , 'LineWidth' , 3);
                ...
                xtick = [];
                for i = [1:length(unique(ANA.Horizon))]
                    plotshade((i-1)*(length(dayz)+1)+[1:length(dayz)],plot1_pred_red(i,:),err1_pred_red(i,:),'transp' , .6 , 'patchcolor' , horzcolor(:,i) , 'linecolor' , colz{6-i+1,2} , 'linewidth' , 3);
                    plot((i-1)*(length(dayz)+1)+[1:length(dayz)],plot1_pred_red(i,:) , '-o' , 'MarkerSize' , 10 , 'color' , colz{6-i+1,2},'MarkerFaceColor',colz{6-i+1,2} , 'LineWidth' , 3);
                    
                    plotshade((i-1)*(length(dayz)+1)+[1:length(dayz)],plot0_pred_red(i,:),err0_pred_red(i,:),'transp' , .6 , 'patchcolor' , horzcolor(:,i) , 'linecolor' , colz{6-i+1,1} , 'linewidth' , 3);
                    plot((i-1)*(length(dayz)+1)+[1:length(dayz)],plot0_pred_red(i,:) , '-o' , 'MarkerSize' , 10 , 'color' , colz{6-i+1,1},'MarkerFaceColor',colz{6-i+1,1} , 'LineWidth' , 3);
                    xtick = [xtick (i-1)*(length(dayz)+1)+[1:length(dayz)]];
                    line([i*(length(dayz)+1) i*(length(dayz)+1)] , [2500 8000] , 'LineWidth' , 3 , 'LineStyle' , ':' , 'color' , [.8 .8 .8])
                end
                set(gca,'FontSize' , 18 , 'XTick' , xtick , ...
    ...
    case 'IPI_asymptote'
        ...
        IPItable.b1 = ones(size(IPItable.IPI));
        IPItable.b2 = ones(size(IPItable.IPI));
        IPItable.b3 = ones(size(IPItable.IPI));
        ...
        for d = 1:length(dayz)
            for subjnum = 1:length(subj_name)-1
                for chp = 0:2
                    ...
                    IPIch.b1 = B1(1)*IPIch.b1;
                    IPIch.b2 = B1(2)*IPIch.b2;
                    IPIch.b3 = B1(3)*IPIch.b3;
                    ...
                    [coo_pred{chp+1}(:,d),plot_pred{chp+1}(:,d),err_pred{chp+1}(:,d)] = lineplot([IPIs.Horizon] , IPIs.ipi_pred , 'subset' , ismember(IPIs.ChunkBndry , chp) & ismember(IPIs.Day , d));
                end
            end
        end
        close(h0)
