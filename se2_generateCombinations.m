function CMB = se1_generateCombinations(varargin)
...
for i = 1:NumQuadro
    ...
    % dont allow equal chunks, mirror chunks, or repetition of the same finger chunks
    while (length(find(temp4(m(i) , :) == temp4(m(i) , 1))) == 4 | sum((temp4(m(i) , :) == mode(temp4(m(i) , :)))) >2 |  ((temp4(m(i) , 1) == temp4(m(i) , 2) & temp4(m(i) , 3)== temp4(m(i) , 4))))
        m(i) = randi([1,length(temp4)], 1);
    end
...
if GenGroupCLADist
    ...
    ChnkPlcmnt = CMB.ChnkPlcmnt(DesInd , :);
    for i  = 1:length(allind)
        a = ChnkPlcmnt(allind(i,:),:);
        for j = 1:length(allind(i,:))
            for k = 1:length(allind(i,:))
                L(j , k) = (a(j,:) - a(k,:)) * (a(j,:) - a(k,:))';
            end
        end
        % set diag(a) to 0 and get the upper traingle
        CMB.CLA_dist(i) = sum(squareform(L));
    end
end
...
CMB.Group1Chnks = ChnkPlcmnt(allind(b(1),:) , :);
CMB.Group1 = allind(b(i),:);
CMB.DesiredCnhkargmnt(allind(b(i),:) , :)

% for group 2 pich the CLAs that chare any of the CLAs in group 1
while i <= length(b)
    if (sum(ismember(allind(b(i),:), allind(b(1),:))) == 0 & length(find(reshape(CMB.DesiredCnhkargmnt(allind(b(i),:) , :) , 42,1))) >=28)
        CMB.Group2Chnks = ChnkPlcmnt(allind(b(i),:) , :);
        break
    end
    i = i +1;
end
